...
public class Proyecto 
{
	...
	public Proyecto() 
	{
		...
		archivo = "./data/archivoBase.txt";
		participantes = new ArrayList<Participante>();
		actividades = new ArrayList<Actividad>();
		actualizador = new Actualizacion(archivo);
	}
	...
	public void setArchivo(String nuevo)
	{
		archivo = nuevo;
		actualizador.setArchivoBase(nuevo);
	}
	...
	public Actividad buscarActividad(String type, String parametro)
	{
		...
		ArrayList<Actividad> encontradas = null;
		if (type.equals("tipo"))
		{
		...
		int choice = Integer.parseInt(input("Por favor seleccione una actividad"));
		
		laDigna = encontradas.get(choice);
		return laDigna;
	}
	...
	public void aEditar(Actividad task, Integer tipoEdicion, String edicion) throws ParseException 
	{
		...
		case 2-> {Date f = sdf.parse(edicion); task.setFechaFinal(f);}
		case 3 ->{task.setTitulo(edicion);}
		case 4 ->{task.setDescripcion(edicion);}
		case 5 ->{task.setTipo(edicion);}
		case 6 ->{Date f = sdf.parse(edicion); task.setFechaFinal(f);}
		}
	} 
